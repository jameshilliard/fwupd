...
#ifdef HAVE_SQLITE
	/* this is generated at runtime or build-time from usb.ids and other static sources */
	if (self->db != NULL) {
...
#ifdef HAVE_SQLITE
	/* this is generated at build-time from usb.ids and other static sources */
	if (self->db != NULL) {
		g_autoptr(sqlite3_stmt) stmt = NULL;
		if (key == NULL) {
			if (sqlite3_prepare_v2(self->db,
			      ...
			      GError **error)
{
	...
	sqlite3_bind_text(helper->stmt, 2, "Vendor", -1, SQLITE_STATIC);
...
fu_quirks_db_load(FuQuirks *self, FuQuirksLoadFlags load_flags, GError **error)
{
	...
	if (sqlite3_exec(self->db,
			 "BEGIN TRANSACTION;"
			 "CREATE TABLE quirks(guid, key, value);"
			 "CREATE INDEX idx_quirks_guid ON quirks(guid);"
			 "CREATE INDEX idx_quirks_guid_key ON quirks(guid, key);"
			 "COMMIT;",
			 NULL,
			 NULL,
			 NULL) != SQLITE_OK) {
		g_set_error(error,
			    ...
			    "failed to create quirk table: %s",
			    sqlite3_errmsg(self->db));
		return FALSE;
	}
...
#ifdef HAVE_SQLITE
	if ((load_flags & FU_QUIRKS_LOAD_FLAG_NO_SQLITE) == 0) {
		...
		if (sqlite3_prepare_v2(self->db, "SELECT key FROM quirks", -1, &stmt, NULL) !=
		    SQLITE_OK) {
			g_debug("quirk sqlite check failed: %s", sqlite3_errmsg(self->db));
