...
typedef struct {
	...
	GPtrArray *interfaces;	      /* of FuUsbInterface */
	GPtrArray *bos_descriptors;   /* of FuUsbBosDescriptor */
...
static gboolean
fu_usb_device_libusb_error_to_gerror(FuUsbDevice *self, gint rc, GError **error)
{
...
#if 0
/**
 * fu_usb_device_get_configuration:
 * @self: a #FuUsbDevice
 ...
 **/
static gint
fu_usb_device_get_configuration(FuUsbDevice *self, GError **error)
{
	FuUsbDevicePrivate *priv = GET_PRIVATE(self);
	gint rc;
	int config;

	g_return_val_if_fail(FU_IS_USB_DEVICE(self), -1);
	...
	if (rc != LIBUSB_SUCCESS) {
		fu_usb_device_libusb_error_to_gerror(self, rc, error);
		return -1;
	}
	return config;
}
...
fu_usb_device_set_dev(FuUsbDevice *self, struct libusb_device *usb_device)
{
	...
	if (usb_device != NULL)
		priv->device = libusb_ref_device(usb_device);
}
...
fu_usb_device_get_parent(FuUsbDevice *self)
{
	...
	if (parent == NULL)
		return NULL;
	return fu_usb_device_new(fu_device_get_context(FU_DEVICE(self)), parent);
}
...
fu_usb_device_claim_interface_cb(FuDevice *device, gpointer user_data, GError **error)
{
	...
	return fu_usb_device_claim_interface(self,
					     iface->number,
					     FU_USB_DEVICE_CLAIM_INTERFACE_BIND_KERNEL_DRIVER,
					     error);
}
...
fu_usb_device_open_internal(FuUsbDevice *self, GError **error)
{
	...
	if (!fu_usb_device_libusb_error_to_gerror(self, rc, error)) {
		if (priv->handle != NULL)
...
fu_usb_device_set_configuration_internal(FuUsbDevice *self, gint configuration, GError **error)
{
	...
	if (rc != LIBUSB_SUCCESS)
		return fu_usb_device_libusb_error_to_gerror(self, rc, error);
...
#if 0
static guint8
fu_usb_device_get_port_number(FuUsbDevice *self)
{
	FuUsbDevicePrivate *priv = GET_PRIVATE(self);
	if (priv->device == 0x0)
		return 0x0;
...
fu_usb_device_setup(FuDevice *device, GError **error)
{
	...
	if (fu_device_get_vendor(device) == NULL) {
		idx = fu_usb_device_get_manufacturer_index(self);
		if (idx != 0x00) {
	...
	if (fu_device_get_name(device) == NULL) {
		idx = fu_usb_device_get_product_index(self);
		if (idx != 0x00) {
			g_autofree gchar *tmp = NULL;
			g_autoptr(GError) error_local = NULL;
			tmp = fu_usb_device_get_string_descriptor(self, idx, &error_local);
...
fu_usb_device_close(FuDevice *device, GError **error)
{
	...
	for (guint i = 0; priv->device_interfaces != NULL && i < priv->device_interfaces->len;
	     i++) {
		FuUsbDeviceInterface *iface = g_ptr_array_index(priv->device_interfaces, i);
		FuUsbDeviceClaimInterfaceFlags claim_flags = FU_USB_DEVICE_CLAIM_INTERFACE_NONE;
		g_autoptr(GError) error_local = NULL;
		if (!iface->claimed)
			continue;
		if (!fu_device_has_flag(device, FWUPD_DEVICE_FLAG_WAIT_FOR_REPLUG)) {
			g_debug("re-binding kernel driver as not waiting for replug");
			claim_flags |= FU_USB_DEVICE_CLAIM_INTERFACE_BIND_KERNEL_DRIVER;
		}
...
fu_usb_device_probe(FuDevice *device, GError **error)
{
	...
	if (priv->device != NULL) {
		g_autofree gchar *platform_id = fu_usb_device_build_physical_id(priv->device);
...
#if 0
/**
 * fu_usb_device_get_device_subclass:
 * @self: a #FuUsbDevice
 *
 * Gets the device subclass qualified by the class number.
 * See fu_usb_device_get_device_class().
 *
 ...
 **/
static guint8
fu_usb_device_get_device_subclass(FuUsbDevice *self)
{
	FuUsbDevicePrivate *priv = GET_PRIVATE(self);
	g_return_val_if_fail(FU_IS_USB_DEVICE(self), 0);
...
/**
 * fu_usb_device_get_device_protocol:
 * @self: a #FuUsbDevice
 *
 * Gets the device protocol qualified by the class and subclass numbers.
 * See fu_usb_device_get_device_class() and fu_usb_device_get_device_subclass().
 *
 ...
 **/
static guint8
fu_usb_device_get_device_protocol(FuUsbDevice *self)
{
	FuUsbDevicePrivate *priv = GET_PRIVATE(self);
	g_return_val_if_fail(FU_IS_USB_DEVICE(self), 0);
...
fu_usb_device_incorporate(FuDevice *device, FuDevice *device_donor)
{
	...
	fu_usb_device_set_dev(self, priv_donor->device);
}
			       ...
			       GError **error)
{
	...
	if (fu_device_has_flag(FU_DEVICE(self), FWUPD_DEVICE_FLAG_EMULATED)) {
		...
		if (rc_tmp != G_MAXINT64)
			return fu_usb_device_libusb_error_to_gerror(self, rc_tmp, error);
		if (actual_length != NULL)
			*actual_length = length;
		return fu_device_event_copy_data(event, "Data", data, length, error);
	}
	...
	/* munge back to flags */
	if (direction == FU_USB_DEVICE_DIRECTION_DEVICE_TO_HOST)
...
fu_usb_device_ensure_interfaces(FuUsbDevice *self, GError **error)
{
	...
	for (guint i = 0; i < config->bNumInterfaces; i++) {
		for (guint j = 0; j < (guint)config->interface[i].num_altsetting; j++) {
			...
			g_ptr_array_add(priv->interfaces, fu_usb_interface_new(ifp));
		}
	}
...
fu_usb_device_get_string_descriptor(FuUsbDevice *self, guint8 desc_index, GError **error)
{
	...
	if (fu_device_has_flag(FU_DEVICE(self), FWUPD_DEVICE_FLAG_EMULATED)) {
		...
		rc_tmp = fu_device_event_get_i64(event, "Error", NULL);
		if (rc_tmp != G_MAXINT64) {
			fu_usb_device_libusb_error_to_gerror(self, rc_tmp, error);
			return NULL;
		}
 ...
 * @flags: #FuUsbDeviceClaimInterfaceFlags
 * @error: a #GError, or %NULL
 *
			      ...
			      GError **error)
{
	...
	if (flags & FU_USB_DEVICE_CLAIM_INTERFACE_BIND_KERNEL_DRIVER) {
...
fu_usb_device_ensure_hid_descriptors(FuUsbDevice *self, GError **error)
{
	...
	for (guint i = 0; i < priv->interfaces->len; i++) {
		...
		g_autoptr(GBytes) blob = NULL;
		if (fu_usb_interface_get_class(intf) != FU_USB_DEVICE_CLASS_HID)
			continue;
...
fu_usb_device_from_json(FwupdCodec *codec, JsonNode *json_node, GError **error)
{
	...
	if (tmp != NULL) {
		...
		fu_device_set_created(FU_DEVICE(self), g_date_time_to_unix(created_new));
	}
...
fu_usb_device_add_json(FwupdCodec *codec, JsonBuilder *builder, FwupdCodecFlags flags)
{
	...
	if (fu_device_get_physical_id(FU_DEVICE(self)) != NULL) {
		json_builder_set_member_name(builder, "PlatformId");
		json_builder_add_string_value(builder, fu_device_get_physical_id(FU_DEVICE(self)));
	}
#if GLIB_CHECK_VERSION(2, 62, 0)
	if (fu_device_get_created(FU_DEVICE(self)) != 0) {
		g_autoptr(GDateTime) dt =
		    g_date_time_new_from_unix_utc(fu_device_get_created(FU_DEVICE(self)));
	...
	if (priv->desc.idVendor != 0) {
		json_builder_set_member_name(builder, "IdVendor");
		json_builder_add_int_value(builder, priv->desc.idVendor);
	}
	if (priv->desc.idProduct != 0) {
		json_builder_set_member_name(builder, "IdProduct");
		json_builder_add_int_value(builder, priv->desc.idProduct);
	}
	if (priv->desc.bcdDevice != 0) {
		json_builder_set_member_name(builder, "Device");
		json_builder_add_int_value(builder, priv->desc.bcdDevice);
	}
	if (priv->desc.bcdUSB != 0) {
		json_builder_set_member_name(builder, "USB");
		json_builder_add_int_value(builder, priv->desc.bcdUSB);
	}
	if (priv->desc.iManufacturer != 0) {
		json_builder_set_member_name(builder, "Manufacturer");
		json_builder_add_int_value(builder, priv->desc.iManufacturer);
	}
	if (priv->desc.bDeviceClass != 0) {
		json_builder_set_member_name(builder, "DeviceClass");
		json_builder_add_int_value(builder, priv->desc.bDeviceClass);
	}
	if (priv->desc.bDeviceSubClass != 0) {
		json_builder_set_member_name(builder, "DeviceSubClass");
		json_builder_add_int_value(builder, priv->desc.bDeviceSubClass);
	}
	if (priv->desc.bDeviceProtocol != 0) {
		json_builder_set_member_name(builder, "DeviceProtocol");
		json_builder_add_int_value(builder, priv->desc.bDeviceProtocol);
	}
	if (priv->desc.iProduct != 0) {
		json_builder_set_member_name(builder, "Product");
		json_builder_add_int_value(builder, priv->desc.iProduct);
	}
	if (priv->desc.iSerialNumber != 0) {
		json_builder_set_member_name(builder, "SerialNumber");
		json_builder_add_int_value(builder, priv->desc.iSerialNumber);
	}
...
static const gchar *
fu_usb_device_class_code_to_string(FuUsbDeviceClassCode code)
{
...
fu_usb_device_to_string(FuDevice *device, guint idt, GString *str)
{
	...
	if (priv->device != NULL) {
		FuUsbDeviceClassCode code = fu_usb_device_get_device_class(self);
